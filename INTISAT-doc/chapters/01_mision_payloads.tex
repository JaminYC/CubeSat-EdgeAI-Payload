...
Following standardized satellite operation approaches, INTISAT's operating layout is defined by distinct mission modes. 
% [Flowchart Placeholder: Flowchart of the Mission State Machine]

\begin{itemize}
    \item \textbf{Deployment \& Initialization Mode:} During launch, all systems are physically disabled by kill-switches. Upon ejection, systems power up. The OBC boots, initial housekeeping checks are executed, and the antenna deployment sequence starts after a mandatory silence window. % [Flowchart Placeholder: Deployment Sequence]
    
    \item \textbf{Nominal Mode:} The standard operational state where the EPS maximizes solar harvesting, the beacon transmits periodically, and the specific payload (Microscopy / AI) is activated during predetermined windows over specific targets or the Peruvian territory. % [Flowchart Placeholder: Normal Beacon and OBDH Operation]
    
    \item \textbf{Contingency Mode:} A safe-state entered automatically if a critical anomaly is detected (e.g., low battery SoC or latch-up events). Non-essential loads are disabled until ground control intervenes.
    
    \item \textbf{Decommissioning Mode:} End-of-life state activated via a ground telecommand or programmed timeout, securing safe passivation of the battery and shutdown of transceivers prior to atmospheric reentry.
\end{itemize}
